...
void SceneWidget::update(phys::Universe &universe, bool should_clear)
{
    ...
    this->renderer.renderBodies(universe.env->getEnvironment_safe(), universe.properties, universe.propertiesIterators,
                                *universe.camera);
    ...
    ImGui::Begin("Bodies");
    {
        ...
        if (ImGui::BeginPopup("Body"))
        {
            ...
            {
                auto err = universe.deleteBody(selected_body_id);
                if (!err.has_value())
                {
                    phys::showMessage(err.error().c_str());
                }
            }
