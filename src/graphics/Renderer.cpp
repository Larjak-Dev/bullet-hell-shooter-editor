...
void Renderer::renderBodies(const EnvironmentBase &env, const Properties &properties,
                            const PropertyIterators &iterators, const Camera &cam, float transparency,
                            Color color_addon)
{
    assert(this->target);
    ...
    renderBodies2D(env, properties, iterators, cam, shader);
                                       ...
                                       const std::vector<std::pair<float, Color>> &properties, const Camera &cam)
{
    ...
    for (auto &&[i, universe] : std::views::enumerate(universes))
    {
        ...
        renderBodies2D(env, universe->properties, universe->propertiesIterators, cam,
...
void Renderer::renderBodies2D(const EnvironmentBase &env, const Properties &properties,
                              const PropertyIterators &iterators, const Camera &cam, gl::ShaderMain &shader)
{
    ...
    {
        const Property &property = *iterators.at(body.id);
        // Model Configuring
