...
void Simulator::advanceEnv(std::shared_ptr<EnvironmentActive> env, const PhysicConfig &physicConfig, double delta_time,
                           StepBuffer &step_buffer)
{
    const auto physic_functions = PhysicFunctions(physicConfig);
    const auto env_new = physic_functions.step(env->getEnvironment_safe(), delta_time, step_buffer);
    env->setEnvironment_safe(env_new);
}
