...
ForceFunction phys::createNullForceFunction()
{
    return ForceFunction(
        [](vec3d pos, const Body &self, const EnvironmentBase &env)
        {
            vec3d F_total{};
            return F_total;
        });
...
StepFunction phys::createImplicitEulerStepFunction(AccelerationFunction accelerationFunction)
{
    return StepFunction(
        [accelerationFunction](const EnvironmentBase &env, double delta_time, StepBuffer &buffer)
        {
            ...
            {
                env_new.bodies.insert(implicitEulerBodyStep(body, delta_time, env, accelerationFunction));
            }
...
StepFunction phys::createVerletStepFunction(AccelerationFunction accelerationFunction)
{
    return StepFunction(
        [accelerationFunction](const EnvironmentBase &env, double delta_time, StepBuffer &buffer)
        {
            ...
            for (const Body &body : env.bodies)
            {
                env_new.bodies.insert(verletBodyStep(body, delta_time, env, accelerationFunction));
            }
